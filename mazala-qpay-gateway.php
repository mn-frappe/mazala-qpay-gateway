 ...
 * Text Domain: mazala-qpay-gateway
...
function mzqpay_woocommerce_missing_notice() {
    ...
    <div class="error notice is-dismissible">
        <p>
            <strong><?php esc_html_e( 'Mazala QPay Gateway', 'mazala-qpay-gateway' ); ?></strong>
            <?php esc_html_e( 'requires WooCommerce to be installed and active.', 'mazala-qpay-gateway' ); ?>
            <a href="<?php echo esc_url( admin_url( 'plugin-install.php?s=woocommerce&tab=search&type=term' ) ); ?>">
                <?php esc_html_e( 'Install WooCommerce', 'mazala-qpay-gateway' ); ?>
            </a>
        </p>
    </div>
    <?php
...
function mzqpay_check_woocommerce_on_activation() {
    if ( ! mzqpay_is_woocommerce_active() ) {
        ...
        wp_die(
            esc_html__( 'Mazala QPay Gateway requires WooCommerce to be installed and active. Please install WooCommerce first.', 'mazala-qpay-gateway' ),
            esc_html__( 'Plugin Activation Error', 'mazala-qpay-gateway' ),
...
function mzqpay_run_activation_tasks() {
    ...
    if ( class_exists( 'ActionScheduler' ) || function_exists( 'as_schedule_recurring_action' ) ) {
        if ( function_exists( 'as_next_scheduled_action' ) && ! as_next_scheduled_action( 'mzqpay_process_queue_hook' ) ) {
            as_schedule_recurring_action( time(), 300, 'mzqpay_process_queue_hook', array(), 'mazala-qpay-gateway' );
        }
    } else {
...
function mzqpay_add_cron_schedule( $schedules ) {
    if ( ! isset( $schedules['mzqpay_every_five_minutes'] ) ) {
        $schedules['mzqpay_every_five_minutes'] = array(
            'interval' => 300,
            'display'  => esc_html__( 'Every 5 Minutes', 'mazala-qpay-gateway' )
        );
    }
    return $schedules;
}
...
function mzqpay_run_deactivation_tasks() {
    ...
    if ( class_exists( 'ActionScheduler' ) && function_exists( 'as_unschedule_all_actions' ) ) {
        as_unschedule_all_actions( 'mzqpay_process_queue_hook', array(), 'mazala-qpay-gateway' );
    }
    // Clear WP Cron
    ...
    if (function_exists('woocommerce_store_api_register_endpoint_data')) {
        woocommerce_store_api_register_endpoint_data([
            ...
            'schema_callback' => function() {
                return [
                    'supports_qr_payment' => [
                        'description' => __('Whether QR code payment is supported', 'mazala-qpay-gateway'),
                        'type'        => 'boolean',
                        'context'     => ['view', 'edit'],
                        'readonly'    => true,
                    ],
                    'supports_bank_apps' => [
                        'description' => __('Whether bank app deep links are supported', 'mazala-qpay-gateway'),
                        'type'        => 'boolean',
                        'context'     => ['view', 'edit'],
                        'readonly'    => true,
                    ],
                    'supports_ebarimt' => [
                        'description' => __('Whether eBarimt tax receipts are supported', 'mazala-qpay-gateway'),
                        'type'        => 'boolean',
                        'context'     => ['view', 'edit'],
                        'readonly'    => true,
                    ],
...
add_filter('plugin_action_links_' . plugin_basename(__FILE__), function($links) {
    ...
    $settings_link = '<a href="' . esc_url($settings_url) . '">' . __('Settings', 'mazala-qpay-gateway') . '</a>';
...
function mzqpay_order_actions($actions){
    $actions['mzqpay_cancel_invoice'] = __('qPay Cancel Invoice','mazala-qpay-gateway');
    $actions['mzqpay_refund_payment'] = __('qPay Refund Payment','mazala-qpay-gateway');
    $actions['mzqpay_cancel_payment'] = __('qPay Cancel Payment (Card only)','mazala-qpay-gateway');
    $actions['mzqpay_cancel_ebarimt'] = __('qPay Cancel eBarimt','mazala-qpay-gateway');
    $actions['mzqpay_get_payment_status'] = __('qPay Get Payment Status','mazala-qpay-gateway');
    return $actions;
}
...
function mzqpay_product_fields(){
    ...
    woocommerce_wp_text_input( array(
        'id' => '_mzqpay_gs1_code',
        'label' => __('qPay GS1 code','mazala-qpay-gateway'),
        'description' => __('Optional: override GS1 classification code for this product (used for eBarimt).','mazala-qpay-gateway'),
        'desc_tip' => true,
    ) );
    ...
    woocommerce_wp_checkbox( array(
        ...
        'label' => __('Disable qPay GS1 auto-mapping','mazala-qpay-gateway'),
        'description' => __('If checked, the gateway will not attempt automatic GS1 mapping for this product.','mazala-qpay-gateway'),
    ) );

    // VAT code for special tax treatment
    $vat_options = array('' => __('Standard VAT (10%)', 'mazala-qpay-gateway'));
    if (class_exists('MZQPay_Fixtures')) {
    ...
    woocommerce_wp_select( array(
        'id' => '_mzqpay_vat_code',
        'label' => __('qPay VAT Code', 'mazala-qpay-gateway'),
        'description' => __('Select VAT treatment for this product (0% or exempt)', 'mazala-qpay-gateway'),
        'desc_tip' => true,
        'options' => $vat_options,
    ) );
