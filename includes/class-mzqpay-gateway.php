...
class MZQPay_Payment_Gateway extends WC_Payment_Gateway {
    ...
    protected $log_context = 'mazala-qpay-gateway';
    
    public function __construct() {
        $this->id = 'mzqpay';
        $this->method_title = 'Mazala QPay Gateway';
        $this->method_description = __('Accept payments via qPay (Mongolian payment gateway) with eBarimt integration.', 'mazala-qpay-gateway');
    ...
    public function init_form_fields(){
        ...
        $district_options = array('' => __('Select District', 'mazala-qpay-gateway'));
        ...
        $bank_options = array('' => __('Select Bank', 'mazala-qpay-gateway'));
        if (class_exists('MZQPay_Fixtures')) {
        ...
        $this->form_fields = array(
            ...
            'test_connection' => array(
                'title' => __('🔌 Connection Status','mazala-qpay-gateway'),
                'type' => 'test_connection',
                'description' => __('Test your qPay API connection','mazala-qpay-gateway'),
            ),
            'enabled' => array(
                'title' => __('Enable qPay','mazala-qpay-gateway'),
                'type' => 'checkbox',
                'label' => __('✓ Accept payments via qPay','mazala-qpay-gateway'),
                'default' => 'yes',
                'description' => __('Enable to start accepting qPay payments immediately in sandbox mode.','mazala-qpay-gateway'),
            ),
            'mode' => array(
                'title' => __('Environment','mazala-qpay-gateway'),
                'type' => 'select',
                'options' => array(
                    'sandbox' => '🧪 Sandbox (Testing)',
                    'production' => '🚀 Production (Live)'
                ),
                'description' => __('Start with Sandbox to test. Switch to Production when ready to accept real payments.','mazala-qpay-gateway'),
                'default' => 'sandbox'
            ),
            
            // Production credentials section - only show in production mode
            'production_section' => array(
                'title' => __('🔐 Production Credentials','mazala-qpay-gateway'),
                'type' => 'title',
                'description' => __('Enter your live qPay credentials (required for production mode).','mazala-qpay-gateway'),
            ),
            'live_client_id' => array(
                'title' => __('Client ID','mazala-qpay-gateway'),
                'type' => 'text',
                'description' => __('Your qPay merchant client ID.','mazala-qpay-gateway'),
                'default' => '',
                'placeholder' => 'Enter your qPay Client ID'
            ),
            'live_client_secret' => array(
                'title' => __('Client Secret','mazala-qpay-gateway'),
                'type' => 'password',
                'description' => __('Your qPay merchant client secret.','mazala-qpay-gateway'),
                'default' => '',
                'placeholder' => 'Enter your qPay Client Secret'
            ),
            'invoice_code' => array(
                'title' => __('Invoice Code','mazala-qpay-gateway'),
                'type' => 'text',
                'description' => __('Your qPay invoice code for production.','mazala-qpay-gateway'),
            ...
            'advanced_section' => array(
                'title' => __('⚙️ Settings Mode','mazala-qpay-gateway'),
                'type' => 'title',
                'description' => '',
            ),
            'advanced_mode' => array(
                'title' => __('Advanced Mode','mazala-qpay-gateway'),
                'type' => 'checkbox',
                'label' => __('Show all settings','mazala-qpay-gateway'),
                'description' => __('Enable to see eBarimt, webhook, and other advanced options.','mazala-qpay-gateway'),
                'default' => 'no'
            ),
        );
        ...
        if ($advanced_mode) {
            $advanced_fields = array(
                ...
                'display_section' => array(
                    'title' => __('📝 Display Settings','mazala-qpay-gateway'),
                    'type' => 'title',
                    'description' => '',
                ),
                'title' => array(
                    'title' => __('Payment Title','mazala-qpay-gateway'),
                    'type' => 'text',
                    'default' => 'qPay',
                    'description' => __('Title shown to customers at checkout.','mazala-qpay-gateway'),
                ),
                'description' => array(
                    'title' => __('Description','mazala-qpay-gateway'),
                    'type' => 'textarea',
                    'default' => 'Pay securely with qPay - QR code or bank app',
                    'description' => __('Description shown to customers at checkout.','mazala-qpay-gateway'),
                ),
                'display_language' => array(
                    'title' => __('Language','mazala-qpay-gateway'),
                    'type' => 'select',
                ...
                'sandbox_section' => array(
                    'title' => __('🧪 Sandbox Credentials','mazala-qpay-gateway'),
                    'type' => 'title',
                    'description' => __('Pre-filled with test credentials. No changes needed for testing.','mazala-qpay-gateway'),
                ),
                'sandbox_client_id' => array(
                    'title' => __('Sandbox Client ID','mazala-qpay-gateway'),
                    'type' => 'text',
                    'default' => 'TEST_MERCHANT',
                    'description' => __('Default: TEST_MERCHANT','mazala-qpay-gateway'),
                ),
                'sandbox_client_secret' => array(
                    'title' => __('Sandbox Client Secret','mazala-qpay-gateway'),
                    'type' => 'password',
                    'default' => '123456',
                    'description' => __('Default: 123456','mazala-qpay-gateway'),
                ),
                
                // eBarimt Settings
                'ebarimt_section' => array(
                    'title' => __('🧾 eBarimt (Tax Receipt)','mazala-qpay-gateway'),
                    'type' => 'title',
                    'description' => __('Configure Mongolian tax receipt generation.','mazala-qpay-gateway'),
                ),
                'enable_ebarimt' => array(
                    'title' => __('Enable eBarimt','mazala-qpay-gateway'),
                    'type' => 'checkbox',
                    'label' => __('Auto-generate eBarimt after payment','mazala-qpay-gateway'),
                    'default' => 'no'
                ),
                'ebarimt_sender_register' => array(
                    'title' => __('Company Register','mazala-qpay-gateway'),
                    'type' => 'text',
                    'description' => __('Your company tax registration number.','mazala-qpay-gateway'),
                    'default' => ''
                ),
                'ebarimt_sender_name' => array(
                    'title' => __('Company Name','mazala-qpay-gateway'),
                    'type' => 'text',
                    'default' => ''
                ),
                'ebarimt_branch_code' => array(
                    'title' => __('Branch Code','mazala-qpay-gateway'),
                    'type' => 'text',
                    'default' => 'SALBAR1'
                ),
                'ebarimt_district_code' => array(
                    'title' => __('District','mazala-qpay-gateway'),
                    'type' => 'select',
                    'options' => $district_options,
                    'default' => ''
                ),
                'ebarimt_receiver_type' => array(
                    'title' => __('Default Receiver Type','mazala-qpay-gateway'),
                    'type' => 'select',
                ...
                'webhook_section' => array(
                    'title' => __('🔔 Webhook Settings','mazala-qpay-gateway'),
                    'type' => 'title',
                    /* translators: %s: callback URL */
                    'description' => sprintf(__('Callback URL: %s','mazala-qpay-gateway'), '<code>' . esc_url(home_url('/?mzqpay_callback=1')) . '</code>'),
                ),
                'webhook_secret' => array(
                    'title' => __('Webhook Secret','mazala-qpay-gateway'),
                    'type' => 'password',
                    'description' => __('Optional: For signature verification.','mazala-qpay-gateway'),
                    'default' => ''
                ),
                
                // Payment Options
                'payment_section' => array(
                    'title' => __('💳 Payment Options','mazala-qpay-gateway'),
                    'type' => 'title',
                    'description' => '',
                ),
                'allow_partial' => array(
                    'title' => __('Partial Payments','mazala-qpay-gateway'),
                    'type' => 'checkbox',
                    'label' => __('Allow partial/installment payments','mazala-qpay-gateway'),
                    'default' => 'no'
                ),
                'enable_expiry' => array(
                    'title' => __('Invoice Expiry','mazala-qpay-gateway'),
                    'type' => 'checkbox',
                    'label' => __('Invoices expire after timeout','mazala-qpay-gateway'),
                    'default' => 'no'
                ),
                
                // Alerts & Logging
                'alerts_section' => array(
                    'title' => __('📊 Monitoring','mazala-qpay-gateway'),
                    'type' => 'title',
                    'description' => '',
                ),
                'enable_logging' => array(
                    'title' => __('Debug Logging','mazala-qpay-gateway'),
                    'type' => 'checkbox',
                    'label' => __('Log to WooCommerce > Status > Logs','mazala-qpay-gateway'),
                    'default' => 'no'
                ),
                'enable_alerts' => array(
                    'title' => __('Email Alerts','mazala-qpay-gateway'),
                    'type' => 'checkbox',
                    'label' => __('Send alerts on payment failures','mazala-qpay-gateway'),
                    'default' => 'no'
                ),
                'alert_email' => array(
                    'title' => __('Alert Email','mazala-qpay-gateway'),
                    'type' => 'text',
                ...
                'env_section' => array(
                    'title' => __('🔧 Developer Options','mazala-qpay-gateway'),
                    'type' => 'title',
                    'description' => __('For advanced server configurations.','mazala-qpay-gateway'),
                ),
                'use_env' => array(
                    'title' => __('Use ENV Variables','mazala-qpay-gateway'),
                    'type' => 'checkbox',
                    'label' => __('Read credentials from environment variables','mazala-qpay-gateway'),
                    'default' => 'no'
                ),
            );
    ...
    public function get_payment($payment_id) {
        if (empty($payment_id)) {
            return new WP_Error('no_payment_id', __('Payment ID is required', 'mazala-qpay-gateway'));
        }
    ...
    public function cancel_ebarimt($ebarimt_id) {
        if (empty($ebarimt_id)) {
            return new WP_Error('no_ebarimt_id', __('eBarimt ID is required', 'mazala-qpay-gateway'));
        }
    ...
    public function process_refund($order_id, $amount = null, $reason = '') {
        ...
        if (!$order) {
            ...
            return new WP_Error('invalid_order', __('Order not found.', 'mazala-qpay-gateway'));
        }
        
        $payment_id = $order->get_meta('_mzqpay_payment_id');
        if (empty($payment_id)) {
            $this->log('Refund failed: No payment ID for order #' . $order_id, 'error');
            return new WP_Error('no_payment_id', __('No qPay payment ID found for this order.', 'mazala-qpay-gateway'));
        }
        ...
        if (is_wp_error($result)) {
            ...
            if ($error_code === 'refund_pending') {
                $order->add_order_note(
                    ...
                    sprintf(__('qPay refund queued for processing. Amount: %1$s. Reason: %2$s', 'mazala-qpay-gateway'), 
        ...
        $order->add_order_note(
            /* translators: 1: refund amount, 2: refund reason */
            sprintf(__('qPay refund processed successfully. Amount: %1$s. Reason: %2$s', 'mazala-qpay-gateway'),
                wc_price($refund_amount, array('currency' => $order->get_currency())),
                $reason
            )
        );
    ...
    protected function build_invoice_lines($order){
        ...
        if ($ship_total > 0){
            ...
            $lines[] = array(
                'line_no' => $i, 
                'description' => __('Shipping', 'mazala-qpay-gateway'), 
                'quantity' => 1, 
    ...
    public function admin_options(){
        ...
        echo '<p>' . esc_html__('qPay v2 gateway settings. Sandbox is prefilled with test credentials on activation.','mazala-qpay-gateway') . '</p>';
    ...
    public function process_payment($order_id){
        ...
        if (!MZQPay_Fixtures::is_currency_supported($currency)) {
            ...
            wc_add_notice(sprintf(__('Currency %1$s is not supported by qPay. Supported currencies: %2$s','mazala-qpay-gateway'), $currency, $supported), 'error');
            return;
        }
        ...
        if (is_wp_error($access_token)){
            ...
            wc_add_notice(__('qPay authentication failed: ','mazala-qpay-gateway') . $error_msg,'error');
            return;
        }
        ...
        if (is_wp_error($inv)){
            ...
            wc_add_notice(__('qPay invoice creation queued for retry','mazala-qpay-gateway'),'notice');
        ...
        if ($response_code >= 400){
            ...
            if (isset($inv_body['message'])) {
                ...
                $error_message = sprintf(__('HTTP %d error','mazala-qpay-gateway'), $response_code);
            }
            ...
            // Enqueue for retry
            $this->enqueue_task(array('type'=>'invoice_create_retry','order_id'=>$order_id,'payload'=>$payload,'attempts'=>0));
            wc_add_notice(__('qPay invoice creation failed: ','mazala-qpay-gateway') . $error_message,'error');
            $order->update_meta_data('_mzqpay_invoice_queued','1');
            $order->save();
            $order->update_status('on-hold', 'Awaiting qPay invoice (queued)');
            return array('result'=>'success','redirect'=>$this->get_return_url($order));
    ...
    public function receipt_page($order){
        echo '<p>' . esc_html__('Thank you. Please wait for qPay payment confirmation.','mazala-qpay-gateway') . '</p>';
        ...
        if ($inv_raw){
            ...
            if (!empty($inv['qr_image'])){
                echo '<p>' . esc_html__('Scan QR to pay:','mazala-qpay-gateway') . '</p>';
                echo '<img src="data:image/png;base64,' . esc_attr($inv['qr_image']) . '" alt="qpay-qr" style="max-width:300px;" />';
            }
            if (!empty($inv['qPay_shortUrl'])){
                echo '<p><a href="' . esc_url($inv['qPay_shortUrl']) . '" target="_blank">' . esc_html__('Open qPay Short URL','mazala-qpay-gateway') . '</a></p>';
            }
            if (!empty($inv['urls']) && is_array($inv['urls'])){
                echo '<p>' . esc_html__('Open in bank app:','mazala-qpay-gateway') . '</p>';
